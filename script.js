...
function initialiserChampsMajuscules() {
  const champs = document.querySelectorAll(".champ-majuscule");
...
function initialiserChampsCapitalises() {
  const champs = document.querySelectorAll(".champ-capitalise");
...
function initialiserDatePlacement() {
  ...
  if (!input.value) {
    input.value = formaterDateFR(new Date());
  }
}
...
function construireDateHeurePlacement() {
  ...
  if (datePlacement && heurePlacement) {
    return `${datePlacement} à ${heurePlacement}`;
  }
...
function viderLesChamps() {
  ...
  document.querySelectorAll('input[type="text"], textarea').forEach((champ) => {
    champ.value = "";
...
if (btnVider) {
  btnVider.addEventListener("click", () => {
    ...
    const confirmation = window.confirm(
      "Vider tous les champs de la fiche en cours ?"
    );
...
if (btnMail) {
  btnMail.addEventListener("click", () => {
    ...
    const sujet = "Placement en Garde à vue";
    const corps = construireTexte();
    const url = `mailto:?subject=${encodeURIComponent(sujet)}&body=${encodeURIComponent(corps)}`;
...
if (btnSMS) {
  btnSMS.addEventListener("click", () => {
    ...
    try {
      const url = `sms:?body=${encodeURIComponent(corps)}`;
      window.location.href = url;
